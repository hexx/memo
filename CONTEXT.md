...
**Auto Title**:
A memo title generated by AI from the memo body. Obtained when the user tries to save with an empty title, or via the "AIで生成" button in the editor. When generation is unavailable (AI not configured or failed), always falls back to manual input and never blocks Memo creation.
